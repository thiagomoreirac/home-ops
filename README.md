...
## <img src="https://fonts.gstatic.com/s/e/notoemoji/latest/1f510/512.gif" alt="🔐" width="20" height="20"> Azure Key Vault: Sync Secrets for Bootstrap

This repository integrates with [Azure Key Vault](https://azure.microsoft.com/en-us/products/key-vault/) to store and retrieve local machine-specific secrets, eliminating the need to recreate `sops`/`age` keys or other credentials on new machines.

### Purpose

Store sensitive local files (age key, kubeconfig, SSH keys, cloudflared tunnel, GitHub credentials, cluster/node configs) in Azure Key Vault so you can:
- Bootstrap new machines idempotently using `scripts/bootstrap.sh`
- Avoid manually recreating the same secrets on every machine
- Rotate and centrally manage credentials
- Use interactive Azure SSO for authentication

### Prerequisites

- **Azure Key Vault**: Create one in Azure (separate from this repo)
- **Azure CLI**: [Install `az`](https://learn.microsoft.com/cli/azure/install-azure-cli)
- **Permissions**: Your Azure identity needs `get`/`list`/`set` secret permissions in the vault (or appropriate RBAC role)
- **Optional**: `task` runner (Taskfile) for convenience; script can be run directly

### Quick Start

1. **Log in to Azure** (interactive SSO):
...
2. **Fetch secrets from Key Vault**:
   ...
   ```bash
   bash scripts/bootstrap.sh --vault MY-VAULT-NAME
   ```

   Or via Taskfile:

   ```bash
   VAULT_NAME=MY-VAULT-NAME USE_KEYVAULT=true task template:keyvault
   ```

3. **Upload local secrets to Key Vault** (if migrating existing credentials):

   ```bash
   bash scripts/bootstrap.sh --vault MY-VAULT-NAME --upload
   ```

### Secret Mapping

By default, `scripts/bootstrap.sh` syncs these secrets:

| Secret Name | Local Path |
|---|---|
...
Customize the mapping by editing `SECRET_TO_PATH` in `scripts/bootstrap.sh`.

### Taskfile Variables

The repository uses centralized variables in `Taskfile.yaml`:

- `VAULT_NAME` — Azure Key Vault name
- `VAULT_RG` — Resource group (optional, for disambiguation)
- `VAULT_SUBSCRIPTION` — Subscription ID (optional)
- `VAULT_URI` — Full vault URI (optional; preferred if set)
- `USE_KEYVAULT` — `true`/`false` to enable Key Vault mode
- `UPLOAD` — `true`/`false` to upload missing local files
- `UPLOAD_ONLY` — `true`/`false` to upload existing files and exit
...
```bash
VAULT_NAME=my-vault VAULT_RG=my-rg USE_KEYVAULT=true task template:keyvault
```

**Upload existing local secrets** (migrate to vault):

```bash
VAULT_NAME=my-vault UPLOAD_ONLY=true task template:keyvault
```

**Generate local age key** (no vault):
...
In CI, use a service principal with vault access:
...
- name: Fetch age key from Key Vault
  run: |
    az keyvault secret show --vault-name my-vault --name age-private-key --query value -o tsv > $HOME/.config/sops/age/keys.txt
    chmod 600 $HOME/.config/sops/age/keys.txt
```

Alternatively, generate the age key locally in CI if vault access is not available:
...
- Keep Key Vault access restricted to necessary principals
- Use vault logging and monitoring to audit access
- Do not store large state files (e.g., Terraform state) in Key Vault — use Azure Storage instead
- Rotate credentials regularly
- Keep the `age` private key secure; it encrypts all `sops` secrets in the repository

### Files & Scripts

- `scripts/bootstrap.sh` — Main script for fetching/uploading secrets
- `.taskfiles/template/Taskfile.yaml` — Task targets for template initialization
- `.taskfiles/bootstrap/Taskfile.yaml` — Task targets for cluster bootstrap
- `.github/workflows/age-key-vault.yaml` — CI workflow example
